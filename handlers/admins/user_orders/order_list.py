...
from models import OrderHistory, Users, CartProducts, Products, Cart


async def user_orders_menu(message: Message):
    ...
    for order in orders:
        ...
        for user in users:
            inline_kb.add(
                InlineKeyboardButton(f'🗙 {user.phone_number} {user.name}', callback_data='choose_order')
            )
...
async def user_orders(message: Message):
    ...
    await message.bot.send_location(chat_id=message.from_user.id, latitude=location_latitude, longitude=location_longitude)
    # for order in orders:
